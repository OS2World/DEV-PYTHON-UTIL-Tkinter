...
PyThreadState *tkThreadState = 0;
int tkThreadId = -1;

void 
tkSaveThread()
{
	tkThreadState = PyEval_SaveThread();
...
tkRestoreThread()
{
	PyEval_RestoreThread(tkThreadState);
	tkThreadState = 0;
}
...
tkThreadOk()
{
	if (tkThreadId == -1) {
		...
		return tkThreadId == tkGetThreadId();
	}
}
	...
	PyObject *args;
{
	...
	if (!cmd)
		...
		PyErr_SetString(Tkinter_TclError,
							 "Tk calls may only be performed from main thread."
							 );

	else {
		int rc;
		tkSaveThread();
		rc = Tcl_Eval(Tkapp_Interp(self), cmd);
		tkRestoreThread();
	...
	PyObject *args;
{
	...
	if (!tkThreadOk()) {
		PyErr_SetString(Tkinter_TclError, 
							 "mainloop() may only be called from main thread."
							 );
		return NULL;
	}
