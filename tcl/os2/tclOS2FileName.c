...
#include "tclInt.h"
#include "tclPort.h"
#include "../generic/tclRegexp.h" /* *MM* added ../generic/ */
...
/*
 * The following regular expression matches the root portion of a OS/2
 * absolute or volume relative path.  It will match both UNC and drive relative
 * paths.
 */

#define OS2_ROOT_PATTERN "^(([a-zA-Z]:)|[/\\][/\\]+([^/\\]+)[/\\]+([^/\\]+)|([/\\]))([/\\])*"
...
static regexp *os2RootPatternPtr = NULL;
 ...
 * of: TCL_PLATFORM_UNIX, TCL_PLATFORM_MAC, TCL_PLATFORM_OS2 or
 * TCL_PLATFORM_OS2.
 */
...
static char *		ExtractOS2Root _ANSI_ARGS_((char *path,
			    Tcl_DString *resultPtr, int offset));
...
static char *		SplitWinPath _ANSI_ARGS_((char *path,
			    Tcl_DString *bufPtr));
static char *		SplitOS2Path _ANSI_ARGS_((char *path,
			    Tcl_DString *bufPtr));
...
{
    if (os2RootPatternPtr != NULL) {
	ckfree((char *)os2RootPatternPtr);
    }
 ...
 * ExtractOS2Root --
 *
 *	Matches the root portion of a OS/2 path and appends it
 *	to the specified Tcl_DString.
 *	
 * Results:
 *	Returns the position in the path immediately after the root
...
static char *
ExtractOS2Root(path, resultPtr, offset)
    char *path;			/* Path to parse. */
    Tcl_DString *resultPtr;	/* Buffer to hold result. */
    int offset;			/* Offset in buffer where result should be
				 * stored. */
{
    ...
    /*
     * Initialize the path name parser for OS/2 path names.
     */

    if (os2RootPatternPtr == NULL) {
	os2RootPatternPtr = TclRegComp(OS2_ROOT_PATTERN);
	if (!initialized) {
	    Tcl_CreateExitHandler(FileNameCleanup, NULL);
	    initialized = 1;
	}
    ...
    /*
     * Match the root portion of a OS/2 path name.
     */

    if (!TclRegExec(os2RootPatternPtr, path, path)) {
	return path;
    }

    Tcl_DStringSetLength(resultPtr, offset);

    if (os2RootPatternPtr->startp[2] != NULL) {
	Tcl_DStringAppend(resultPtr, os2RootPatternPtr->startp[2], 2);
	if (os2RootPatternPtr->startp[6] != NULL) {
	    Tcl_DStringAppend(resultPtr, "/", 1);
	}
    } else if (os2RootPatternPtr->startp[4] != NULL) {
	Tcl_DStringAppend(resultPtr, "//", 2);
	length = os2RootPatternPtr->endp[3]
	    - os2RootPatternPtr->startp[3];
	Tcl_DStringAppend(resultPtr, os2RootPatternPtr->startp[3], length);
	Tcl_DStringAppend(resultPtr, "/", 1);
	length = os2RootPatternPtr->endp[4]
	    - os2RootPatternPtr->startp[4];
	Tcl_DStringAppend(resultPtr, os2RootPatternPtr->startp[4], length);
    } else {
	Tcl_DStringAppend(resultPtr, "/", 1);
    }
    return os2RootPatternPtr->endp[0];
}

/*
 *----------------------------------------------------------------------
    ...
    char *path;
{
	...
	case TCL_PLATFORM_OS2:
		...
		if (TclRegExec(os2RootPatternPtr, path, path)) {
		    if (os2RootPatternPtr->startp[5]
			    || (os2RootPatternPtr->startp[2]
				    && !(os2RootPatternPtr->startp[6]))) {
			type = TCL_PATH_VOLUME_RELATIVE;
#ifdef DEBUG
                        printf("    TCL_PATH_VOLUME_RELATIVE\n");
		    } else {
                        printf("    (else) TCL_PATH_ABSOLUTE\n");
#endif
		    }
		} else {
		    type = TCL_PATH_RELATIVE;
#ifdef DEBUG
                    printf("    TCL_PATH_RELATIVE\n");
#endif
		}
	    }
	    break;
    ...
    Tcl_DString *bufPtr;	/* Pointer to DString to use for the result. */
{
    ...
    p = ExtractOS2Root(path, bufPtr, 0);
    ...
    Tcl_DString *resultPtr;	/* Pointer to previously initialized DString. */
{
	...
	case TCL_PLATFORM_OS2:
	    ...
	    for (i = 0; i < argc; i++) {
		p = ExtractOS2Root(argv[i], resultPtr, oldLength);
		length = Tcl_DStringLength(resultPtr);
				 ...
				 * not even be initialized. */
{
    ...
    /*
     * Convert forward slashes to backslashes in OS/2 paths because
     * some system interfaces don't accept forward slashes.
     */

    if (tclPlatform == TCL_PLATFORM_OS2) {
